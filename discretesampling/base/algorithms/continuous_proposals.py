...
def grid_search_logprobs(data_function, stan_model, grid_size, min_vals, max_vals, offsets, inds, current_continuous,
    ...
    # max_vals           - list of maximum values of each constrained parameter in order that they appear in
    #                      current_continuous
    # offsets            - randomly sampled offsets used to define the grids
    ...
    param_length = stan_model.num_unconstrained_parameters(proposed_data)
    ...
    for n in range(0, num_params):
        step = (max_vals[n] - min_vals[n]) / (grid_size + 1)
        grid_axes.append(np.linspace(offsets[n], step*(grid_size - 1) + offsets[n], grid_size))
    ...
    for n in range(0, num_grid_points):
        ...
        lp_vals[n] = stan_model.eval(proposed_data, proposed_continuous[0:param_length])[0]
        proposals.append(proposed_continuous)
    ...
    norm_lp_vals = lp_vals - max_lp
    logprobs = norm_lp_vals - logsumexp(norm_lp_vals)
...
def reverse_grid_search(data_function, stan_model, grid_size, min_vals, max_vals, inds, current_continuous, current_discrete,
    ...
    param_length = stan_model.num_unconstrained_parameters(current_data)
    ...
    for param_num in range(0, num_params):
        ...
        grid_axes.append(np.linspace(offset, step*(grid_size - 1) + offset, grid_size))
        param_ind += dim
    ...
    for n in range(0, num_grid_points):
        ...
        for param_num in range(0, num_params):
            ...
            param_ind1 += dim - 1
        lp_vals[n] = stan_model.eval(current_data, proposed_continuous[0:param_length])[0]
