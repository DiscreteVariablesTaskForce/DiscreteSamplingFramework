...
class DiscreteVariable:
    ...
    @classmethod
    def getProposalType(self) -> Type['DiscreteVariableProposal']:
    ...
    @classmethod
    def getTargetType(self) -> Type['DiscreteVariableTarget']:
    ...
    @classmethod
    def getLKernelType(self) -> Type['DiscreteVariableProposal']:
    ...
    @classmethod
    def getOptimalLKernelType(self) -> Type['DiscreteVariableProposal']:
    ...
    def decode(self, x: np.ndarray, particle: 'DiscreteVariable') -> 'DiscreteVariable':
        """Decode an encoded `DiscreteVariable`.

        Decodes an encoded `DiscreteVariable` (for example, encoded by
        `encode`) back to a `DiscreteVariable`.
...
class DiscreteVariableProposal:
    ...
    def norm(self, x: DiscreteVariable) -> float:
        ...
        ----------
        x : DiscreteVariable
            DiscreteVariable for which norm should be calculated

        Returns
        -------
        float
            Norm metric of given DiscreteVariable `x`
        ...
        This metric is not necessarily bijective, i.e. `norm(x)` is not necessarily unique to all `x`.
        For example, for `discretesampling.domain.decision_tree.Tree`, the `norm` is simply the number of nodes in the tree.
    ...
    def heuristic(self, x: Union[int, float, list, tuple], y: Union[int, float, list, tuple]) -> bool:
        """Calculate heuristic for proposal between norms generated from two instances of `DiscreteVariable`

        Parameters
        ----------
        x : Union[int, float, list, tuple]
            Norm of first DiscreteVariable
        y : Union[int, float, list, tuple]
            Norm of second DiscreteVariable

        Returns
        -------
        bool
            Heuristic value between two instances `DiscreteVariable` with norms x and y

        See Also
        --------
        norm : Calculate norm value for `DiscreteVariable`

        Notes
        -----
        Should return true if proposal is possible between x and y (and possibly at other times) where x and y are
        norm values generated by `norm`, but will only return false if the proposal between x and y is definitely impossible.
...
class DiscreteVariableTarget:
    ...
    def evaluatePrior(self, x: DiscreteVariable) -> float:
        ...
        Notes
        -----
            Generally a target/posterior distribution cannot be written explicitly with a closed form solution, and we instead
            write the posterior :math:`P(\theta | X )` as proportional to the product of the likelihood
            :math:`P(X | \theta)` and the prior :math:`P(\theta)`.
