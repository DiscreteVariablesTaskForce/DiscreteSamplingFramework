...
class stats():
    ...
    def predict(self, X_test, use_majority=True):
        ...
        all_possibilities_from_all_trees = []
        leaf_possibilities_for_all_trees = []
        leaf_predictions_for_all_trees = []# the leaf we ended up to make a prediction for one tree
        for tree in self.trees:
            all_labels_from_this_trees, per_tree_possibilities, leaf_poss, leaf_pred = self.predict_for_one_tree(tree, X_test)
            leaf_possibilities_for_all_trees.append(leaf_poss)
            all_labels_from_all_trees.append(all_labels_from_this_trees)
            all_possibilities_from_all_trees.append(per_tree_possibilities)
            leaf_predictions_for_all_trees.append(leaf_pred)
        if use_majority:
            return self.majority_voting_predict(all_labels_from_all_trees), all_possibilities_from_all_trees, leaf_possibilities_for_all_trees, leaf_predictions_for_all_trees
        else:
    ...
    def predict_for_one_tree(self, tree, X_test):
        ...
        all_probs_per_tree = []
        leaf_predictions_for_one_tree = [] # the leaf we ended up to make a prediction for one tree
        for datum in X_test:
            label_for_this_datum, prob, leaf  = self.predict_for_one_datum(tree, leafs, leaf_possibilities, datum)
            all_labels_for_this_tree.append(label_for_this_datum)
            all_probs_per_tree.append(prob)
            leaf_predictions_for_one_tree.append(leaf)
        return all_labels_for_this_tree,all_probs_per_tree, leaf_possibilities, leaf_predictions_for_one_tree
